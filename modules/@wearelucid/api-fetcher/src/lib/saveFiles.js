 ...
 * Save files
 */
export default function saveFiles(data, bundleName, config, additionalNaming) {
  ...
  const fileName = `${bundleName}${data.language ? `.${data.language}` : ''}${
    additionalNaming ? `.${additionalNaming}` : ''
  }.json`
