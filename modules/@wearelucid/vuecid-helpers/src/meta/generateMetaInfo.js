...
export default function generateMetaInfo(options, post, locale, route) {
  ...
  const ogSiteName = options.localized.og_site_name
    ? options.localized.og_site_name
    : siteName
  const title = generateMetaTitle(options, post)
  const description = generateMetaDescription(options, post)
  const ogTitle =
    post && post.og_enabled && post.og_title ? post.og_title : title
  const ogDescription =
    post && post.og_enabled && post.og_description
      ? post.og_description
      : description
  ...
  const canonicalUrl =
    websiteUrl + removeLeadingSlash(removeTrailingSlash(route))
